...
const AdminMenu = () => {
  ...
  const [itemForm, setItemForm] = useState({
    itemName: '', itemOnlinePrice: '', itemOnlineDisplayName: '', itemId: '', categoryId: '', dietaryTag: '', rankOrder: 1, allowVariations: false
  });
  ...
  const handleItemSubmit = async (e) => {
    ...
    try {
      ...
      setEditingItem(null);
      setItemForm({ itemName: '', itemOnlinePrice: '', itemOnlineDisplayName: '', itemId: '', categoryId: '', dietaryTag: '', rankOrder: 1, allowVariations: false });
      fetchData();
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-xl bg-black/60">
          <div className="bg-[#121214] border border-white/10 rounded-[2.5rem] w-full max-w-sm overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="p-8 pb-4 border-b border-white/10">
              <h2 className="text-3xl font-serif font-bold text-white">New Category</h2>
            </div>
            <form onSubmit={handleCategorySubmit} className="p-8 space-y-6">
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button type="button" onClick={() => setCategoryModalOpen(false)} className="px-6 py-2.5 rounded-xl text-xs font-bold uppercase bg-white/5 hover:bg-white/10">Cancel</button>
                <button type="submit" className="px-6 py-2.5 rounded-xl text-xs font-black uppercase bg-primary text-black shadow-lg">Create Category</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <header className="mb-8 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              className="flex items-center gap-2 px-5 py-2.5 bg-white/5 border border-white/10 rounded-xl font-bold text-xs uppercase tracking-wider hover:bg-white/10 transition-all text-white/70"
              onClick={() => { setCategoryForm({ name: '', icon: '☕' }); setCategoryModalOpen(true); }}
            >
            ...
            <button
              ...
              onClick={() => { setEditingItem(null); setItemForm({ itemName: '', itemOnlinePrice: '', itemOnlineDisplayName: '', itemId: '', categoryId: '', dietaryTag: '', rankOrder: 1, allowVariations: false }); setItemModalOpen(true); }}
            >

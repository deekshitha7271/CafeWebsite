...
import { Sparkles, Crown, Activity, Coffee, Moon, ArrowRight, Camera, MessageCircle, Mail, MapPin, Clock, Zap, Leaf, Smartphone, Check, Flame, Plus, ChevronLeft, ChevronRight } from 'lucide-react';
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-10 mb-20">
            ...
            <div className="overflow-x-auto hide-scrollbar flex space-x-3 p-2 bg-surface-dark/40 backdrop-blur-2xl rounded-[30px] border border-white/5 w-full md:w-auto overflow-y-hidden touch-pan-x">
              <button
                ...
                className={`whitespace-nowrap px-8 py-3.5 rounded-[22px] text-[10px] uppercase tracking-[0.2em] font-black transition-all flex-shrink-0 ${activeCategory === 'all'
              ...
              {categories.map(cat => (
                <button
                  ...
                  className={`whitespace-nowrap px-8 py-3.5 rounded-[22px] text-[10px] uppercase tracking-[0.2em] font-black transition-all flex items-center gap-3 flex-shrink-0 ${activeCategory === cat._id
                    ? 'bg-primary text-background shadow-[0_10px_30px_rgba(245,158,11,0.4)]'
                    : 'text-text-muted hover:text-white'
                    }`}
                >
            ...
            <div className="mt-20 flex flex-col items-center gap-6">
              <div className="flex items-center gap-3">
                <motion.button
                  ...
                  className="w-14 h-14 rounded-2xl bg-surface-dark border border-white/10 flex items-center justify-center text-white disabled:opacity-30 disabled:cursor-not-allowed hover:border-primary/50 transition-all"
                >
                  <ChevronLeft className="w-6 h-6" />
                </motion.button>

                <div className="flex items-center gap-2 px-6 py-3 bg-surface-dark/40 backdrop-blur-xl rounded-2xl border border-white/5">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-text-muted">
                Showing {startIndex + 1}-{Math.min(startIndex + itemsPerPage, filteredItems.length)} of {filteredItems.length} delicacies
              </p>
            </div>
          )}
        </section>
